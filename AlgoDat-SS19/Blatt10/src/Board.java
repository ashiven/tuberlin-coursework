...
/**
 * Board is a class which represents a board of a puzzle game, called 15-puzzle
 * (or 8-puzzle),
 * see <a href="https://en.wikipedia.org/wiki/15_puzzle">the wikipedia
 * article</a>. Here, the
 * size of the board is not fixed, and can even be a non-square rectangle.
 * <p>
 * There is one particularity in the implementation. For performance reason, the
 * positions of
 * a board are stored in a <em>static</em> variable. Since those positions are
 * dependend on the
 * size of the board, this imposes the restriction that all objects existing
 * simultaneously have
...
public class Board {
    ...
    // Variable positions is declared static for performance reason (see comment in
    // the docstring)
    ...
    /**
     * Constructor generates an empty board of the given diemnsions (<em>nx</em>
     * horizontally and
     * <em>ny</em> vertically.
     *
    ...
    /**
     * Constructor which reads a {@link Board} from a text file. The first line has
     * the
     * horizontal <em>nx</em> and the vertical <em>ny</em> dimensions of the field.
     * Each of the following lines
     * defines the fields of one row. No check for consistency is performed. The
     * tokens have
    ...
    /**
     * Main part of the constructor. This is put in a separate method to avoid that
     * the code
     * needs to be double. The {@code Board(String)} constructor cannot call the
     * basic constructor
     * since that is allowed as the first line only, and the size of the board needs
     * to be read
     * from the file first.
     */
    ...
    /**
     * Return the content of a field (token number, or {@code Board.BLANK} if the
     * field is free,
     * or {@code Board.OFF} is the specified {@link Position} does not correspond to
     * a field on
     * the board.
     *
     ...
     * @param token The token (number between 1 and {@code nx*ny - 1} that is to be
     *              placed,
     *              or {@code Board.BLANK} to clear a field.
     * @throws InputMismatchException for invalid <em>pos</em> or <em>token</em> out
     *                                of range
     */
    ...
    /**
     * Returns all possible moves for the current board as {@code Iterable}. The
     * move that would undo
     * the move that is provided are argument <em>lastMove</em> is excluded. If
     * {@code lastMove}
     * is {@code null}, no move is excluded.
     *
     * @param {@link Move} lastMove Last move in the move sequence: the inverse of
     *               this move is excluded.
     * @return all possible moves as {@code Iterable<Move>}, excluding the inverse
     *         of <em>lastMove</em>
     */
    ...
    /**
     * Checks whether the state of the board is the solved state, i.e. the number
     * are
    ...
    /**
     * Computes the Manhattan distance (aka taxicab metric), see
     * <a href="https://en.wikipedia.org/wiki/Taxicab_geometry">here</a>,
     * between the current puzzle configuration and the goal configuration: For each
     * token, the distance from its
     * curent position to its position in the solution is calculated in x- and
     * y-direction. These
     * two (absolute) values are added for each token. The sum across all tokens
     * (not for the blank
    ...
    public int manhattan() {
        ...
        for (int i = 0; i < this.nx; i++) {
            for (int j = 0; j < this.ny; j++) {
                if ((field[j][i] - 1) % nx != i && field[j][i] != 0) {
                    diffx += Math.abs(i - (field[j][i] - 1) % nx);
                }
                if ((field[j][i] - 1) / nx != j && field[j][i] != 0) {
                    diffy += Math.abs(j - (field[j][i] - 1) / nx);
                }
                /*
                 * if(field[j][i] == 0 && j != this.ny - 1 && i != this.nx - 1) {
                 * diffx += Math.abs(i - (this.nx - 1));
                 * diffy += Math.abs(j - (this.ny - 1));
                 * }
                 */
            }
        }
