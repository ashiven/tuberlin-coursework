...
def create_pod_spec(workload_name, node_name, execution_time):
    ...
    pod_spec['spec']['containers'][0]['command'][-1] = f"sysbench cpu --cpu-max-prime=10000 --time={execution_time} run"
...
def main():
    ...
    while True:
        ...
        logging.info(f"New workload: {workload_name}, Carbon Intensity: {intensity}, Recommended Node: {best_node}")
