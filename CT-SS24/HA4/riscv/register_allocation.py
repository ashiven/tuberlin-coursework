...
############################## REGISTER ALLOCATION ###############################
##################################################################################
# - Calculates the number of ssa values/registers to be spilled for a given function (i.e. the main function)
# - Also calculates the number of ssa values/registers to be allocated on the stack
# - One spill per function argument and ssa result and one stack allocation for each AllocOp

#  Returns:
#  number of spills, {operation - ssa/register index}, number of allocs, {allocop - ssa/register index}

REAL_REGISTERS = 10
REGISTER_OFFSET = 32 - (
    REAL_REGISTERS + 4
)  # using the registers 18-27 because t3-t6 are used by functions like print etc
USE_REGISTERS = True
...
##################################################################################
############################## Rewrite Pass from RiscSSA to Risc #################
##################################################################################
...
class RiscvToRiscvSSAPattern(RewritePattern):
    ...
    def get_stack_pos(self, val: SSAValue):
        """
        Return the stack offset for a given SSA value, including whether it is found in the local scope.
        """
    ...
    def add_stack_allocation(self, spilled_reg: int, stack_vars: int):
        """
        ...
        idx = 0
        for arg in self.func.func_body.blocks[0].args:
            header_ops += self.spill_from(RegisterAttr.from_name(f"a{idx}"), arg)
            idx += 1

        footer_ops: List[Operation] = [
            riscv.CommentOp(""),
            riscv.CommentOp("Footer Ops"),
            riscv.LabelOp("_" + self.func.properties["func_name"].data + "_return"),
            riscv.AddIOp(
                "sp",
                "sp",
                4 * spilled_reg,
        ...
        block = self.func.regions[0].blocks[0]
        if block.first_op:
    ...
    def unspill_to(self, val: SSAValue, reg: Register) -> List[Operation]:
        ...
        # get the stack position of the value and whether it is part of the local stack scope
        stack_pos, is_local = self.get_stack_pos(val)

        # unspilling a value from the local or global scope
        op = riscv.LWOp(
            reg,
            "sp" if is_local else "tp",
            stack_pos * 4,
            f"Unspill register '{formatted_op}'",
        )

        if USE_REGISTERS:
            # the value is in the global scope when
            # (1) it is in the local stack of the main function - signified by use_registers
            # (2) it is in the global stack - signified by not is_local
            if is_local and self.is_main or not is_local:
                # we are trying to access a value from the global scope that is in a register
                if stack_pos < REAL_REGISTERS:
                    op = riscv.AddIOp(
                        reg,
                        RegisterAttr.from_index(stack_pos + REGISTER_OFFSET),
                        0,
                        f"Load variable '{formatted_op}'",
                    )
                # we are trying to access a value from the global scope that is not in a register
                else:
                    pos = stack_pos - REAL_REGISTERS
                    op = riscv.LWOp(
                        reg, "tp", pos * 4, f"Unspill register '{formatted_op}'"
                    )

        return [op]

    def spill_from(self, reg: Register, val: SSAValue) -> List[Operation]:
        """
        Store a variable into its place in the stack, knowing the
        current position of the variable in the registers.
        """
        pos = (
            self.stack_pos[val] - REAL_REGISTERS
            if self.is_main and USE_REGISTERS
            else self.stack_pos[val]
        )
        if pos * 4 > 2**12:
            raise NotImplementedError(
                "Register allocator is not working for more than 128 variables."
    ...
    def rewrite_ecall(self, op: riscvssa.ECALLOp, rewriter: PatternRewriter) -> None:
        new_ops = self.unspill_to(op.syscall_num, RegisterAttr.from_name("a7"))
        for idx, operand in enumerate(op.args):
            new_ops.extend(
                self.unspill_to(operand, RegisterAttr.from_name("a" + str(idx)))
            )
    ...
    def rewrite_call(self, op: riscvssa.CallOp, rewriter: PatternRewriter) -> None:
        ...
        if len(op.results) == 1:
            rd_stack_pos = self.get_stack_pos(op.results[0])[0]
            if rd_stack_pos < REAL_REGISTERS and self.is_main and USE_REGISTERS:
                new_ops.append(
                    riscv.AddIOp(
                        RegisterAttr.from_index(rd_stack_pos + REGISTER_OFFSET),
                        RegisterAttr.from_name("a0"),
                        0,
                        "Save return value into register",
                    )
                )
            else:
                new_ops.extend(
                    self.spill_from(RegisterAttr.from_name("a0"), op.results[0])
                )
        rewriter.replace_op(op, new_ops, [None] * len(op.results), safe_erase=True)
    ...
    def rewrite_alloc(self, op: riscvssa.AllocOp, rewriter: PatternRewriter) -> None:
        stack_alloc_pos = (
            4
            * (
                self.alloc_to_stack_var[op]
                + (
                    len(self.stack_pos) - REAL_REGISTERS
                    if len(self.stack_pos) > REAL_REGISTERS
                    else 0
                )
            )
            if self.is_main and USE_REGISTERS
            else 4 * (self.alloc_to_stack_var[op] + len(self.stack_pos))
        )
        ...
        if rd_stack_pos < REAL_REGISTERS and self.is_main and USE_REGISTERS:
            ...
            new_ops.extend(self.spill_from(RegisterAttr.from_name("t0"), op.results[0]))

        rewriter.replace_op(op, new_ops, [None] * len(op.results), safe_erase=True)

    def rewrite_return(self, ret: riscvssa.ReturnOp, rewriter: PatternRewriter):
        new_ops = self.unspill_to(ret.value, RegisterAttr.from_name("a0"))
        new_ops.append(
    ...
    def match_and_rewrite(self, op: Operation, rewriter: PatternRewriter):
        ...
        # This is the Risc Op corresponding to the given RiscSSA Op
        new_op_type = self.ctx.get_op("riscv." + opname)

        # Adds a comment op to signify which RiscSSA Op was translated
        full = self.output.getvalue()
        ...
        new_ops = (
            []
        )  # TODO: add comments back after everything is done [riscv.CommentOp(""), riscv.CommentOp(f"{formatted_op}")]
        ...
        # Here we either unspill rs1 and rs2 or we load them from the correct registers
        if len(op.operands) > 0:
            rs1_stack_pos = self.get_stack_pos(op.operands[0])[0]
            if rs1_stack_pos < REAL_REGISTERS and self.is_main and USE_REGISTERS:
                new_op_properties["rs1"] = RegisterAttr.from_index(
                    rs1_stack_pos + REGISTER_OFFSET
                )
            else:
                new_ops.extend(
                    self.unspill_to(op.operands[0], RegisterAttr.from_name("t1"))
                )
                new_op_properties["rs1"] = RegisterAttr.from_name("t1")
        if len(op.operands) > 1:
            rs2_stack_pos = self.get_stack_pos(op.operands[1])[0]
            if rs2_stack_pos < REAL_REGISTERS and self.is_main and USE_REGISTERS:
                new_op_properties["rs2"] = RegisterAttr.from_index(
                    rs2_stack_pos + REGISTER_OFFSET
                )
            else:
                new_ops.extend(
                    self.unspill_to(op.operands[1], RegisterAttr.from_name("t2"))
                )
                new_op_properties["rs2"] = RegisterAttr.from_name("t2")
        if len(op.results) != 0:
            rd_stack_pos = self.get_stack_pos(op.results[0])[0]
            if rd_stack_pos < REAL_REGISTERS and self.is_main and USE_REGISTERS:
                new_op_properties["rd"] = RegisterAttr.from_index(
                    rd_stack_pos + REGISTER_OFFSET
                )
            else:
        ...
        # This is the store instruction that gets added after every op with an ssa result
        # Used for the purpose of spilling the ssa value onto the stack
        if len(op.results) != 0:
            rd_stack_pos = self.get_stack_pos(op.results[0])[0]
            if rd_stack_pos < REAL_REGISTERS and self.is_main and USE_REGISTERS:
                pass
                ...
                new_ops.extend(
                    self.spill_from(RegisterAttr.from_name("t0"), op.results[0])
                )
...
class RISCVSSAToRISCV(ModulePass):
    ...
    def apply(self, ctx: MLContext, mod: ModuleOp) -> None:
        ...
        for func in main.func_body.ops:
            ...
            pattern.add_stack_allocation(spilled_reg, stack_vars)
            add_return(func)
        ...
        # add instructions before and after the function to allocate and free stack space for spilling
        if USE_REGISTERS:
            if global_spilled_reg > REAL_REGISTERS:
                global_spilled_reg -= REAL_REGISTERS
                pattern.add_stack_allocation(global_spilled_reg, global_stack_vars)
            else:
                pattern.add_stack_allocation(0, global_stack_vars)
        else:
            pattern.add_stack_allocation(global_spilled_reg, global_stack_vars)
        ...
        # only add the functions that are used in the program
        used_funcs = set()
        for op in main.func_body.ops:
            if isinstance(op, riscv.JALOp):
                used_funcs.add(op.offset.data)
            elif isinstance(op, riscv.BEQOp):
                used_funcs.add(op.offset.data)
            elif isinstance(op, riscv.BGEUOp):
                used_funcs.add(op.offset.data)
            elif isinstance(op, riscvssa.FuncOp):
                for op in op.func_body.ops:
                    if isinstance(op, riscv.JALOp):
                        used_funcs.add(op.offset.data)
                    elif isinstance(op, riscv.BEQOp):
                        used_funcs.add(op.offset.data)
        ...
        if "_print_int" in used_funcs:
            add_print(mod)
        if "_print_bool" in used_funcs:
            add_print_bool(mod)
        if "_print_str" in used_funcs:
            add_print_string(mod)
        if "_input" in used_funcs:
            add_input(mod)
        if "_list_concat" in used_funcs:
            add_list_concat(mod)
        if "_str_eq" in used_funcs:
            add_str_eq(mod)
        if "_error_len_none" in used_funcs:
            add_print_error(
                mod,
                "_error_len_none",
                "TypeError: object of type 'NoneType' has no len()",
            )
        if "_list_index_oob" in used_funcs:
            add_print_error(
                mod, "_list_index_oob", "IndexError: list index out of range"
            )
        if "_list_index_none" in used_funcs:
            add_print_error(
                mod,
                "_list_index_none",
                "TypeError: 'NoneType' object is not subscriptable",
            )
        if "_error_div_zero" in used_funcs:
