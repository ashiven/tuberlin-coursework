...
int peer_is_responsible(uint16_t pred_id, uint16_t peer_id, uint16_t hash_id) {
    if(pred_id < hash_id && hash_id <= peer_id) {
        return 1;
    }
    //in case pred is the last node in the ring and peer is the first one
    if(pred_id > peer_id) {
        //the value is NOT in the interval [peer_id,...,pred_id] thus it is among the remaining values which peer is responsible for
        if(!(peer_id < hash_id && hash_id <= pred_id)) {
            return 1;
        }
    }
    return 0;
