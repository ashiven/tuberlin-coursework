...
int main(int argc, char **argv) {
    ...
    // Allow client to send finger packets for debugging 
    if(strcmp(method, "FINGER") == 0) {
        p->flags = PKT_FLAG_CTRL | PKT_FLAG_FNGR; // Alle anderen werte in p sind egal 
    ...
    if (!(rsp->flags & PKT_FLAG_ACK) && !(rsp->flags & PKT_FLAG_FACK)) {
